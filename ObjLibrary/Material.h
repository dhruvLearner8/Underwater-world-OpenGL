...
//    <3> isValidIlluminationMode(m_illumination_type)
...
//    or shading.  The sum of the ambient and diffuse colours
//    is used (some files use one, some use the other).
//
//  All non-shader drawing while GL_LIGHTING is disabled is
//    performed as if using this illumination mode (other than
//    materials with ILLUMINATION_CONSTANT_ADDITIVE).  If you
//    are using display lists, this is determined when the
...
//
//  ILLUMINATION_CONSTANT_ADDITIVE
//
//  The material should use additive transparency, without
//    lighting or shading.  The sum of the ambient and diffuse
//    colours is used (some files use one, some use the other).
//
//  Meshes with this material ignore lighting and shading.
//
//  This does not correspond to any MTL illumination mode.
//
	static const unsigned int
	                        ILLUMINATION_CONSTANT_ADDITIVE = 11;
...
//  Side Effect: N/A
//
	bool isDisplayTexturesLoaded () const;  //  SHOULD THIS BE ONLY FOR NON-SHADER?  <|>
...
//           specified file.  The file will be a MTL library
//           file containing only this Material.
//  Parameter(s):
...
//               lost.  This Material is then written to that
//               file in MTL format.  Any saving errors are
//               written to the standard error stream (cerr).
//
