...
//    <3> m_file_name_lowercase == toLowercase(m_file_name)
//    <4> m_file_path_lowercase == toLowercase(m_file_path)
...
//  save
//
//  Purpose: To write the contents of this MtlLibrary to a file
//           with the current file name and path.  In most
//           cases, this is the file this MtlLibrary was loaded
//           from.
//  Parameter(s): N/A
//  Precondition(s): N/A
//  Returns: N/A
//  Side Effect: A file with the current file name and path
//               (if any) is created.  If a file by that name
//               already exists, its contents are lost.  This
//               MtlLibrary is then written to that file in MTL
//               format.  Any errors are written to the standard
//               error stream (cerr).
//
	void save () const;
...
//    <2> logfile: The file to write saving errors to
//    <2> r_logstream: The stream to write saving errors to
//  Precondition(s):
//    <1> ObjStringParsing::isValidFilenameWithPath(filename)
//    <2> ObjStringParsing::isValidFilenameWithPath(logfile)
//  Returns: N/A
//  Side Effect: A file named filename is created.  If a file by
//               that name already exists, its contents are
//               lost.  This MtlLibrary is then written to that
//               file in MTL format.  If a logfile or logging
//               stream is specified, any saving errors are
//               written to that file or stream.  Otherwise, any
//               saving errors are written to the standard error
//               stream (cerr).
//
...
//  setFileName
//
//  Purpose: To change the file name of this MtlLibrary.
//  Parameter(s):
//    <1> filename: The new file name
//  Precondition(s):
//    <1> name != ""
//  Returns: N/A
//  Side Effect: This MtlLibrary is set to have file name
...
//  Parameter(s):
//    <1> material: The Material to add
//  Precondition(s):
//    <1> !isMaterial(material.getName())
//  Returns: The index of the new Material.
//  Side Effect: A copy of Material material is added to this
//               MtlLibrary.  This MtlLibrary does not store a
//               pointer/reference to material and will not
//               delete it.
//
	unsigned int add (const Material& material);
...
//  Returns: The index of the new Material.
//  Side Effect: Material p_material is added to this
//               MtlLibrary.  p_material is assumed to be
//               dynamically allocated and will eventually be
//               destroyed by this MtlLibrary.
//
