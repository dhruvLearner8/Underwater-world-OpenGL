...
bool SpriteFont :: isGlutInitialized ()
{
...
{
	assert(isGlutInitialized());
	assert(ObjStringParsing::isValidFilenameWithPath(image));
	assert(red != green || red != blue);
...
SpriteFont :: ~SpriteFont ()
{
	...
	//  -> not checking if they are 0, those are silently ignored
...
bool SpriteFont :: isInitialized () const
{
                                          ...
                                          unsigned int format) const
{
	...
	int total_length = 0;
	for(unsigned int current = 0; current < (unsigned int)(str.size()); current++)
	{
		...
		{
			if(current + 1 == (unsigned int)(str.size()))
                         ...
                         unsigned char blue)
{
	...
	if(font.isBad())
	{
		cerr << "Error: Could not initialize font because texture didn't load" << endl;
		assert(invariant());
		return;
	}
...
#ifdef OBJ_LIBRARY_SHADER_DISPLAY
	for(unsigned int block_start = 0; block_start < (unsigned int)(str.size()); block_start += BLOCK_SIZE_MAX)
	{
		// draw very long strings in smaller blocks of characters
		assert(block_start < (unsigned int)(str.size()));
		unsigned int block_size = (unsigned int)(str.size()) - block_start;
