...
namespace
{
	...
	//
	//  This variable has to by dynamically alloated so that it
	//    is not destroyed when the program terminates.
	//    -> If it is destroyed, the destructor tries to free
	//       the graphics memory, and that can cause a crash if
	//       the OpenGL context was destroyed first
	//    -> Destruction order seems to be unpredictable
	//
	Texture* gp_white;
	...
	const Texture& getDummyTexture ()
	{
		...
		{
			TextureBmp white;
			gp_white = new Texture(white.addToOpenGL());
		}

		assert(gp_white != NULL);
		return *gp_white;
	}
	...
	//  -> and as locals in the big variant of load
...
{
	return (unsigned int)(gvp_textures.size());
}
...
const std::string& getName (unsigned int index)
{
	...
	assert(index < (unsigned int)(gvp_textures.size()));
...
unsigned int TextureManager :: getIndex (const std::string& name)
{
	...
	for(unsigned int i = 0; i < (unsigned int)(gvp_textures.size()); i++)
	{
...
unsigned int TextureManager :: add (const Texture& texture, const string& name)
{
	...
	unsigned int texture_count = (unsigned int)(gvp_textures.size());

	// we could make an initializing constructor for TextureData, but what's the point?
	gvp_textures.push_back(new TextureData);
	assert(texture_count < (unsigned int)(gvp_textures.size()));
                                     ...
                                     ostream& r_logstream)
{
	...
	//  All gets, activates, and loads without
	//    -> wrapping and mag/minification parameters
	//    -> transparency
...
#else
	return load(name, GL_REPEAT, GL_REPEAT, GL_NEAREST, GL_NEAREST_MIPMAP_NEAREST, r_logstream);
#endif
}
                                     ...
                                     const Vector3& transparent_colour)
{
	...
	return load(string(a_name), transparent_colour, cerr);
}
                                     ...
                                     const string& logfile)
{
	...
	ofstream logstream(logfile.c_str());
	return load(string(a_name), transparent_colour, logstream);
	// logstream.close() will be called by ofstream destructor
}
                                     ...
                                     ostream& r_logstream)
{
	...
	return load(string(a_name), transparent_colour, r_logstream);
}
                                     ...
                                     const Vector3& transparent_colour)
{
	...
	return load(name, transparent_colour, cerr);
}
                                     ...
                                     const string& logfile)
{
	...
	ofstream logstream(logfile.c_str());
	return load(name, transparent_colour, logstream);
	// logstream.close() will be called by ofstream destructor
}
                                     ...
                                     ostream& r_logstream)
{
	...
	//  All gets, activates, and loads with transparency but without
	//    -> wrapping and mag/minification parameters
	//  go through this line.
#ifdef OBJ_LIBRARY_LINEAR_TEXTURE_INTERPOLATION
	return load(name, GL_REPEAT, GL_REPEAT, GL_LINEAR, GL_LINEAR_MIPMAP_LINEAR, transparent_colour, r_logstream);
#else
	return load(name, GL_REPEAT, GL_REPEAT, GL_NEAREST, GL_NEAREST_MIPMAP_NEAREST, transparent_colour, r_logstream);
#endif
}
