...
//
//  ObjSettings.h may be #included by header file.  It is not
//    also #included here so it doesn't have to be removed twice
//    when using this class without the ObjLibrary.
//

#include "ObjSettings.h"

#define GL_SILENCE_DEPRECATION
// needs to be after #including ObjSettings.h so macro is defined
...
namespace
{
	...
	//  Parameter(s):
	//    <1> r_output_file: A reference to the binary input
	//                       file to write to
...
bool TextureBmp :: isGlutInitialized ()
{
...
unsigned int TextureBmp :: loadTexture (const char* a_filename)
{
	assert(isGlutInitialized());
	assert(a_filename != NULL);

	return loadTexture(string(a_filename));
}

unsigned int TextureBmp :: loadTexture (const string& filename)
{
	assert(isGlutInitialized());
                                        ...
                                        unsigned char invisible_blue)
{
	...
	return loadTexture(string(a_filename), invisible_red, invisible_green, invisible_blue);
}
                                        ...
                                        unsigned char invisible_blue)
{
	...
	TextureBmp image_alpha(image_basic,
	                       0, 0,
	                       image_basic.getWidth(), image_basic.getHeight(),
	                       invisible_red, invisible_green, invisible_blue);
	assert(!image_alpha.isBad());
	return image_alpha.addToOpenGL();
}

void TextureBmp :: loadTextureArray (const char* a_filename,
                                     ...
                                     unsigned int a_names[])
{
	...
	loadTextureArray(string(a_filename),
	                 textures_x,        textures_y,
	                 texture_width,     texture_height,
	                 texture_spacing_x, texture_spacing_y,
	                 a_names);
}

void TextureBmp :: loadTextureArray (const string& filename,
                                     unsigned int textures_x,
                                     unsigned int textures_y,
                                     unsigned int texture_width,
                                     unsigned int texture_height,
                                     ...
                                     unsigned int a_names[])
{
	...
	unsigned int seperation_x = texture_width  + texture_spacing_x;
	unsigned int seperation_y = texture_height + texture_spacing_y;
	unsigned int total_required_width  = textures_x * seperation_x - texture_spacing_x;
	unsigned int total_required_height = textures_y * seperation_y - texture_spacing_y;
...
unsigned int TextureBmp :: loadTexture2dArray (const char* a_filename,
                                               unsigned int textures_x,
                                               unsigned int textures_y,
                                               unsigned int texture_width,
                                               unsigned int texture_height,
                                               ...
                                               unsigned int texture_spacing_y)
{
	...
	return loadTexture2dArray(string(a_filename),
	                          textures_x,        textures_y,
	                          texture_width,     texture_height,
	                          texture_spacing_x, texture_spacing_y);
}

unsigned int TextureBmp :: loadTexture2dArray (const string& filename,
                                               unsigned int textures_x,
                                               unsigned int textures_y,
                                               unsigned int texture_width,
                                               unsigned int texture_height,
                                               ...
                                               unsigned int texture_spacing_y)
{
	...
	unsigned int layer_count  = textures_x * textures_y;
	unsigned int seperation_x = texture_width  + texture_spacing_x;
	unsigned int seperation_y = texture_height + texture_spacing_y;
	unsigned int total_required_width  = textures_x * seperation_x - texture_spacing_x;
	unsigned int total_required_height = textures_y * seperation_y - texture_spacing_y;
	...
	//
	//  A GL_TEXTURE_2D_ARRAY is essentially a 3D texture that
	//    is accessed differently.  Each Z level stores a
	//    seperate 2D image.  There is no Z interpolation.
	// 
	...
	//glGenerateMipmap(GL_TEXTURE_2D_ARRAY);  // don't need, no mipmaps
...
TextureBmp :: TextureBmp (unsigned int width, unsigned int height, bool is_alpha)
{
	assert(width > 0);
	assert(height > 0);

	md_texture = NULL;
	init(width, height, is_alpha);
...
TextureBmp :: TextureBmp (unsigned int width,
                          unsigned int height,
                          bool is_alpha,
                          ...
                          unsigned char a_texture[])
{
	...
	assert(width > 0);
	assert(height > 0);
	assert(!is_alpha || array_size == getBytesPerRowAlpha(width)   * height);
	assert( is_alpha || array_size == getBytesPerRowNoAlpha(width) * height);
...
TextureBmp :: TextureBmp (const TextureBmp& source,
                          unsigned int x,
                          unsigned int y,
                          unsigned int width,
                          unsigned int height)
{
	...
	m_width = width;
	m_height = height;
	m_is_alpha = source.isAlphaChannel();
                          ...
                          unsigned char invisible_blue)
{
	...
	assert(x + width <= source.getWidth());
	assert(y + height <= source.getHeight());
	assert(source.isAlphaChannel() == false);
...
unsigned int TextureBmp :: getRGB (unsigned int x, unsigned int y) const
{
	...
	else
	{
		...
		unsigned char red   = md_texture[index + 0];
		unsigned char green = md_texture[index + 1];
		unsigned char blue  = md_texture[index + 2];
		unsigned int rgb = (unsigned int)(red)   <<  0 |
		                   (unsigned int)(green) <<  8 |
		                   (unsigned int)(blue)  << 16;
		return rgb;

		// NO: goes 1 byte out of array on last element
...
{
	assert(a_filename != NULL);
	assert(ObjStringParsing::isValidFilenameWithPath(a_filename));
...
void TextureBmp :: init (unsigned int width, unsigned int height, bool is_alpha)
{
	assert(width > 0);
	assert(height > 0);
	...
	// set all memory to 0
	memset(md_texture, 0, m_array_size);
...
void TextureBmp :: mirrorX ()
{
	for(unsigned int x1 = 0; x1 < m_width / 2; x1++)
	{
		...
		for(unsigned int y = 0; y < m_height; y++)
		{
			unsigned int rgb1 = getRGB(x1, y);
			unsigned int rgb2 = getRGB(x2, y);
			setPixel(x1, y, rgb2);
			setPixel(x2, y, rgb1);
		}
	}
...
void TextureBmp :: mirrorY ()
{
	for(unsigned int y1 = 0; y1 < m_height / 2; y1++)
	{
		...
		for(unsigned int x = 0; x < m_width; x++)
		{
			unsigned int rgb1 = getRGB(x, y1);
			unsigned int rgb2 = getRGB(x, y2);
			setPixel(x, y1, rgb2);
			setPixel(x, y2, rgb1);
		}
	}
...
void TextureBmp :: rotateCW ()
{
	TextureBmp rotated(getHeight(), getWidth(), isAlphaChannel());

	for(unsigned int y1 = 0; y1 < m_height; y1++)
	{
		unsigned int y2 = m_height - 1 - y1;
		for(unsigned int x = 0; x < m_width; x++)
		{
			unsigned int rgb = getRGB(x, y1);
			rotated.setPixel(y2, x, rgb);
		}
	}
...
void TextureBmp :: rotateCCW ()
{
	TextureBmp rotated(getHeight(), getWidth(), isAlphaChannel());

	for(unsigned int x1 = 0; x1 < m_width; x1++)
	{
		unsigned int x2 = m_width - 1 - x1;
		for(unsigned int y = 0; y < m_height; y++)
		{
			unsigned int rgb = getRGB(x1, y);
			rotated.setPixel(y, x2, rgb);
		}
	}
...
void TextureBmp :: rotate180 ()
{
	for(unsigned int y1 = 0; y1 < m_height / 2; y1++)
	{
		...
		{
			unsigned int x2 = m_width - 1 - x1;
			unsigned int rgb1 = getRGB(x1, y1);
			unsigned int rgb2 = getRGB(x2, y2);
			setPixel(x1, y1, rgb2);
			setPixel(x2, y2, rgb1);
		}
	}

	// flip middle row if size is odd
	if(m_height % 2 != 0)
	{
		unsigned int y = m_height / 2;
		for(unsigned int x1 = 0; x1 < m_width / 2; x1++)
		{
			unsigned int x2 = m_width - 1 - x1;
			unsigned int rgb1 = getRGB(x1, y);
...
void TextureBmp :: setPixel (unsigned int x,
                             unsigned int y,
                             unsigned char red,
                             unsigned char green,
                             unsigned char blue)
                             ...
                             unsigned int rgb)
{
	...
	unsigned char red   = (unsigned char)( rgb        & 0xFF);
	unsigned char green = (unsigned char)((rgb >> 8 ) & 0xFF);
	unsigned char blue  = (unsigned char)((rgb >> 16) & 0xFF);

	if(isAlphaChannel())
	{
		unsigned char alpha = (unsigned char)((rgb >> 24) & 0xFF);
...
void TextureBmp :: fill (unsigned char red,
                         unsigned char green,
                         unsigned char blue)
{
	assert(!isAlphaChannel());
...
void TextureBmp :: fill (unsigned int rgb)
{
	unsigned char red   = (unsigned char)( rgb        & 0xFF);
	unsigned char green = (unsigned char)((rgb >> 8 ) & 0xFF);
	unsigned char blue  = (unsigned char)((rgb >> 16) & 0xFF);
...
unsigned int TextureBmp :: addToOpenGL (unsigned int wrap_s,
                                        ...
                                        unsigned int min_filter) const
{
	...
	{
#ifndef OBJ_LIBRARY_SHADER_DISPLAY
		// old way of doing mipmaps

		//
		//  Creating mipmaps seems to scale you to a power-of-2
		//    texture automatically.
		//   -> but after truncating to 4 bytes width
		//
...
#else
		// new way of doing mipmaps

		// first load the texture without mipmaps
		if(m_is_alpha)
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_width, m_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, md_texture);
		else
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB,  m_width, m_height, 0, GL_RGB,  GL_UNSIGNED_BYTE, md_texture);

		// then generate the mipmaps
...
void TextureBmp :: copy (const TextureBmp& original)
{
	...
	m_is_alpha = original.m_is_alpha;
...
bool TextureBmp :: invariant () const
{
	...
	if(!m_is_alpha && m_bytes_per_row != getBytesPerRowNoAlpha(m_width)) return false;
	if( m_is_alpha && m_bytes_per_row !=   getBytesPerRowAlpha(m_width)) return false;
