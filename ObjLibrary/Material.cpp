...
bool Material :: isValidIlluminationMode (unsigned int mode)
{
	...
	case ILLUMINATION_CONSTANT_ADDITIVE:
		return true;
	default:
		return false;
	}
...
void Material :: activate () const
{
	...
	if(effective_illumination_mode == ILLUMINATION_CONSTANT_ADDITIVE)
	{
		glDisable(GL_LIGHTING);
		glBlendFunc(GL_SRC_ALPHA, GL_ONE);
	}
	else if(glIsEnabled(GL_LIGHTING) == GL_FALSE)
	...
	//
	//  11  Color only, additive transparency
	//
	...
	if(effective_illumination_mode != ILLUMINATION_CONSTANT &&
	   effective_illumination_mode != ILLUMINATION_CONSTANT_ADDITIVE)
	{
...
MaterialForShader Material :: getForShader () const
{
	...
	{
	case ILLUMINATION_CONSTANT:
	case ILLUMINATION_CONSTANT_ADDITIVE:  //  ADD  <|>
		//
		//  We use the sum of the ambient, diffuse, and emisive
		//    colours because we don't know what the colour will
		//    be in the MTL file as.
		//
...
bool Material :: invariant () const
{
	...
	if(!isValidIlluminationMode(m_illumination_mode)) return false;
