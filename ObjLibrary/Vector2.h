...
//
  ...
  #define VECTOR2_IS_FINITE(n) ((n) <= DBL_MAX && (n) >= -DBL_MAX)
#endif
...
//
	bool isZero () const
	{
		if(std::fabs(x) > VECTOR2_ZERO_TOLERENCE) return false;
		if(std::fabs(y) > VECTOR2_ZERO_TOLERENCE) return false;
		return true;
	}
...
//  Side Effect: N/A
//
	Vector2 getRejection (const Vector2& project_onto) const
	{
		assert(isFinite());
		assert(project_onto.isFinite());
		assert(!project_onto.isZero());
...
//           (e.g. "1, 2") or values in tuple notation
//           (e.g. "(1, 2)").
//  Parameter(s):
...
//           specified Vector2.
//  Parameter(s):
//    <1> vector: The Vector2
//  Precondition(s): N/A
//  Returns: A Vector2 containing the components
//           (floor(vector.x), floor(vector.y)).
//  Side Effect: N/A
//
inline Vector2 floor (const Vector2& vector)
{
	return Vector2(std::floor(vector.x),
	               std::floor(vector.y));
}
...
//  Precondition(s): N/A
//  Returns: A Vector2 containing the components
//           (ceil(vector.x), ceil(vector.y)).
//  Side Effect: N/A
//
inline Vector2 ceil (const Vector2& vector)
{
	return Vector2(std::ceil(vector.x),
	               std::ceil(vector.y));
}
...
//           Vector2 containing the components
...
{
#if __cplusplus >= 201103L  // C++11
	return Vector2(std::round(vector.x),
	               std::round(vector.y));
#else
...
//           in the specified Vector2.
//  Parameter(s):
//    <1> vector: The Vector2
//  Precondition(s): N/A
//  Returns: A Vector2 containing the components
//           (fabs(vector.x), fabs(vector.y)).
//  Side Effect: N/A
//
inline Vector2 abs (const Vector2& vector)
{
	return Vector2(std::fabs(vector.x),
	               std::fabs(vector.y));
}
inline Vector2 fabs (const Vector2& vector)
{
	return abs(vector);
}
...
//           specified Vector2 with the specified scalar value.
//  Parameter(s):
...
//  Side Effect: N/A
//
inline Vector2 fmod (const Vector2& dividend,
                     double divisor)
{
	return Vector2(std::fmod(dividend.x, divisor),
...
//           of components in the specified Vector2s.
//  Parameter(s):
//    <1> dividend: The dividend
//    <2> divisor: The divisor
//  Precondition(s):
...
//
inline Vector2 fmod (const Vector2& dividend,
                     const Vector2& divisor)
{
	return Vector2(std::fmod(dividend.x, divisor.x),
...
//           value with each component in the specified Vector2.
//  Parameter(s):
//    <1> dividend: The dividend
//    <2> divisor: The divisor
//  Precondition(s):
...
//  Side Effect: N/A
//
inline Vector2 fmod (double dividend,
                     const Vector2& divisor)
{
	return Vector2(std::fmod(dividend, divisor.x),
