...
namespace
{
	...
	#ifdef OBJ_LIBRARY_SHADER_DISPLAY
		// unused file scope constants cause warnings on XCode
...
ObjModel :: ObjModel (const string& filename)
		: mv_material_libraries(),
		  ...
		  mv_meshes()
{
	assert(ObjStringParsing::isValidFilenameWithPath(filename));
...
{
	assert(ObjStringParsing::isValidFilenameWithPath(filename));
	assert(ObjStringParsing::isValidFilenameWithPath(logfile));
...
{
	return (unsigned int)(mv_material_libraries.size());
}
...
string ObjModel :: getMaterialLibraryPath (unsigned int library) const
{
	...
	assert(library < (unsigned int)(mv_material_libraries.size()));
...
{
	return (unsigned int)(mv_vertexes.size());
}
...
{
	return (unsigned int)(mv_texture_coordinates.size());
}
...
{
	return (unsigned int)(mv_normals.size());
}
...
{
	return (unsigned int)(mv_meshes.size());
}
...
unsigned int ObjModel :: getPointSetCount (unsigned int mesh) const
{
	...
	return (unsigned int)(mv_meshes[mesh].mv_point_sets.size());
}
...
unsigned int ObjModel :: getPointSetVertexCount (unsigned int mesh, unsigned int point_set) const
{
	...
	return (unsigned int)(mv_meshes[mesh].mv_point_sets[point_set].mv_vertexes.size());
}
...
unsigned int ObjModel :: getPolylineCount (unsigned int mesh) const
{
	...
	return (unsigned int)(mv_meshes[mesh].mv_polylines.size());
}
...
unsigned int ObjModel :: getPolylineVertexCount (unsigned int mesh, unsigned int polyline) const
{
	...
	return (unsigned int)(mv_meshes[mesh].mv_polylines[polyline].mv_vertexes.size());
}
...
bool ObjModel :: isPolylineTextureCoordinatesAny (unsigned int mesh, unsigned int polyline) const
{
	...
	for(unsigned int i = 0; i < (unsigned int)(v_vertexes.size()); i++)
...
unsigned int ObjModel :: getFaceCount (unsigned int mesh) const
{
	...
	return (unsigned int)(mv_meshes[mesh].mv_faces.size());
}
...
unsigned int ObjModel :: getFaceVertexCount (unsigned int mesh, unsigned int face) const
{
	...
	return (unsigned int)(mv_meshes[mesh].mv_faces[face].mv_vertexes.size());
}
...
bool ObjModel :: isMeshTextureCoordinatesAny (unsigned int mesh) const
{
	...
	for(unsigned int f = 0; f < (unsigned int)(v_faces.size()); f++)
	{
...
unsigned int ObjModel :: getMeshPointCountTotal (unsigned int mesh) const
{
	...
	for(unsigned int i = 0; i < (unsigned int)(v_point_sets.size()); i++)
		total += (unsigned int)(v_point_sets[i].mv_vertexes.size());
	return total;
}
...
{
	unsigned int total = 0;
	for(unsigned int i = 0; i < (unsigned int)(mv_meshes.size()); i++)
		total += (unsigned int)(mv_meshes[i].mv_point_sets.size());
	return total;
}
...
unsigned int ObjModel :: getPolylineCountTotal () const
{
	unsigned int total = 0;
	for(unsigned int i = 0; i < (unsigned int)(mv_meshes.size()); i++)
		total += (unsigned int)(mv_meshes[i].mv_polylines.size());
	return total;
}
...
unsigned int ObjModel :: getFaceCountTotal () const
{
	unsigned int total = 0;
	for(unsigned int i = 0; i < (unsigned int)(mv_meshes.size()); i++)
		total += (unsigned int)(mv_meshes[i].mv_faces.size());
	return total;
}

bool ObjModel :: isAllTriangles () const
{
	for(unsigned int m = 0; m < (unsigned int)(mv_meshes.size()); m++)
...
bool ObjModel :: isSingleMaterial () const
{
	...
	for(unsigned int i = 1; i < (unsigned int)(mv_meshes.size()); i++)
...
{
	unsigned int mesh_count = (unsigned int)(mv_meshes.size());
...
void ObjModel :: print (ostream& r_logstream) const
{
	...
	{
		unsigned int library_count = (unsigned int)(mv_material_libraries.size());
		r_logstream << "    " << library_count << " material libraries" << endl;
		for(unsigned int m = 0; m < library_count; m++)
...
void ObjModel :: printMtlLibraries (ostream& r_logstream) const
{
	...
	for(unsigned int m = 0; m < (unsigned int)(mv_material_libraries.size()); m++)
		r_logstream << "\t\"" << mv_material_libraries[m].m_file_name << "\"" << endl;
}
...
{
	assert(isValid());
	assert((unsigned int)(materials.size()) >= getMeshCount());
...
{
	assert(isValid());
	assert((unsigned int)(va_names.size()) >= getMeshCount());
...
{
	assert(isValid());
	assert((unsigned int)(v_names.size()) >= getMeshCount());
...
DisplayList ObjModel :: getDisplayList () const
{
	...
	//    any MTL file referenced by this ObjModel would be
                                                ...
                                                bool is_normals) const
{
	...
	for(unsigned int m = 0; m < (unsigned int)(mv_meshes.size()); m++)
	{
		...
		// add polylines
		for(unsigned int l = 0; l < (unsigned int)(mv_meshes[m].mv_polylines.size()); l++)
		{
...
void ObjModel :: save () const
{
	// validity of file name and path is checked by class invariant
	save(m_file_path + m_file_name, cerr);
}
...
void ObjModel :: save (const string& filename, ostream& r_logstream) const
{
	...
	if(isAllTriangles())
		output_file << "#  All faces are triangulated" << endl;
	output_file << "#" << endl;
	output_file << endl;
	...
	{
		unsigned int library_count = (unsigned int)(mv_material_libraries.size());
		output_file << "# " << library_count << " material libraries" << endl;
		output_file << "mtllib";
		for(unsigned int m = 0; m < library_count; m++)
			output_file << " " << mv_material_libraries[m].m_file_name;
		output_file << endl;  // ends previous line
		output_file << endl;
	...
	{
		output_file << "# " << getVertexCount() << " vertexes" << endl;
		for(unsigned int v = 0; v < (unsigned int)(mv_vertexes.size()); v++)
	...
	{
		output_file << "# " << getTextureCoordinateCount() << " texture coordinate pairs" << endl;
		for(unsigned int t = 0; t < (unsigned int)(mv_texture_coordinates.size()); t++)
	...
	{
		output_file << "# " << getNormalCount() << " vertex normals" << endl;
		for(unsigned int n = 0; n < (unsigned int)(mv_normals.size()); n++)
	...
	if(mv_meshes.size() > 0)
	{
		...
		for(unsigned int m = 0; m < (unsigned int)(mv_meshes.size()); m++)
		{
			...
			if(mv_meshes[m].mv_point_sets.size() > 0)
			{
				...
				for(unsigned int p = 0; p < (unsigned int)(mv_meshes[m].mv_point_sets.size()); p++)
				{
					output_file << "p";
					for(unsigned int i = 0; i < (unsigned int)(mv_meshes[m].mv_point_sets[p].mv_vertexes.size()); i++)
			...
			if(mv_meshes[m].mv_polylines.size() > 0)
			{
				...
				{
					output_file << "l";
					for(unsigned int i = 0; i < (unsigned int)(mv_meshes[m].mv_polylines[l].mv_vertexes.size()); i++)
					{
			...
			if(mv_meshes[m].mv_faces.size() > 0)
			{
				...
				for(unsigned int f = 0; f < (unsigned int)(mv_meshes[m].mv_faces.size()); f++)
				{
					output_file << "f";
					for(unsigned int i = 0; i < (unsigned int)(mv_meshes[m].mv_faces[f].mv_vertexes.size()); i++)
					{
...
void ObjModel :: setFileName (const string& filename)
{
	assert(ObjStringParsing::isValidFilename(filename));
...
void ObjModel :: setFileNameWithPath (const string& filename)
{
	...
	ObjStringParsing::separatePathOutOfFilename(filename, m_file_name, m_file_path);
	assert(ObjStringParsing::isValidFilename(m_file_name));
...
void ObjModel :: setMeshMaterial (unsigned int mesh, const string& material)
{
	...
	for(unsigned int i = 0; i < (unsigned int)(mv_material_libraries.size()); i++)
	{
...
{
	assert(ObjStringParsing::isValidFilenameWithPath(library));
...
void ObjModel :: addMaterialLibrary (const string& library, ostream& r_logstream)
{
	...
	{
		assert(mv_material_libraries.size() >= 1);
		unsigned int index = (unsigned int)(mv_material_libraries.size()) - 1;
...
{
	unsigned int id = (unsigned int)(mv_vertexes.size());
...
{
	unsigned int id = (unsigned int)(mv_texture_coordinates.size());
...
unsigned int ObjModel :: addNormal (const Vector3& normal)
{
	...
	unsigned int id = (unsigned int)(mv_normals.size());
...
{
	unsigned int id = (unsigned int)(mv_meshes.size());
...
unsigned int ObjModel :: addPointSet (unsigned int mesh)
{
	...
	unsigned int id = (unsigned int)(mv_meshes[mesh].mv_point_sets.size());
...
unsigned int ObjModel :: addPointSetVertex (unsigned int mesh, unsigned int point_set, unsigned int vertex)
{
	...
	unsigned int id = (unsigned int)(mv_meshes[mesh].mv_point_sets[point_set].mv_vertexes.size());
...
unsigned int ObjModel :: addPolyline (unsigned int mesh)
{
	...
	unsigned int id = (unsigned int)(mv_meshes[mesh].mv_polylines.size());
...
unsigned int ObjModel :: addPolylineVertex (unsigned int mesh, unsigned int polyline, unsigned int vertex, unsigned int texture_coordinates)
{
	...
	unsigned int id = (unsigned int)(mv_meshes[mesh].mv_polylines[polyline].mv_vertexes.size());
...
unsigned int ObjModel :: addFace (unsigned int mesh)
{
	...
	unsigned int id = (unsigned int)(mv_meshes[mesh].mv_faces.size());
...
unsigned int ObjModel :: addFaceVertex (unsigned int mesh, unsigned int face, unsigned int vertex, unsigned int texture_coordinates, unsigned int normal)
{
	...
	unsigned int id = (unsigned int)(mv_meshes[mesh].mv_faces[face].mv_vertexes.size());
                                 ...
                                 unsigned int point_set)
{
	...
	unsigned int point_set_count = (unsigned int)(mv_meshes[mesh].mv_point_sets.size());
                                       ...
                                       unsigned int vertex)
{
	...
	unsigned int vertex_count = (unsigned int)(rv_vertexes.size());
                                 ...
                                 unsigned int polyline)
{
	...
	unsigned int polyline_count = (unsigned int)(mv_meshes[mesh].mv_polylines.size());
                             ...
                             unsigned int face)
{
	...
	unsigned int face_count = (unsigned int)(rv_faces.size());
...
void ObjModel :: validate (bool is_print_invalid)
{
...
/*
		...
		if(mv_meshes[m].m_material_name != "" && mv_meshes[m].mp_material == NULL)
		{
			if(is_print_invalid || DEBUGGING_VALIDATE)
				cout << "Invalid material in \"" << m_file_name << "\": Mesh " << s << " has non-existant material \"" << mv_meshes[m].m_material_name << "\"" << endl;
			m_valid = false;
...
*/
		for(unsigned int p = 0; p < (unsigned int)(mv_meshes[m].mv_point_sets.size()); p++)
		{
			unsigned int point_set_vertex_count = (unsigned int)(mv_meshes[m].mv_point_sets[p].mv_vertexes.size());
			if(point_set_vertex_count < 1)
			{
				if(is_print_invalid || DEBUGGING_VALIDATE)
					cout << "Invalid vertex in \"" << m_file_name << "\": Mesh " << m << ", point set " << p << " has only " << point_set_vertex_count << " vertexes (min 1)" << endl;
				m_valid = false;
			...
			for(unsigned int v = 0; v < point_set_vertex_count; v++)
			{
				...
				{
					if(is_print_invalid || DEBUGGING_VALIDATE)
						cout << "Invalid vertex in \"" << m_file_name << "\": Point Set " << p << ", vertex " << v << " has vertex " << (vertex + 1) << " / " << getVertexCount()  << endl;
					m_valid = false;
		...
		for(unsigned int l = 0; l < (unsigned int)(mv_meshes[m].mv_polylines.size()); l++)
		{
			unsigned int polyline_vertex_count = (unsigned int)(mv_meshes[m].mv_polylines[l].mv_vertexes.size());
			if(polyline_vertex_count < 2)
			{
				if(is_print_invalid || DEBUGGING_VALIDATE)
					cout << "Invalid vertex in \"" << m_file_name << "\": Mesh " << m << ", polyline " << l << " has only " << polyline_vertex_count << " vertexes (min 2)" << endl;
				m_valid = false;
			...
			for(unsigned int v = 0; v < polyline_vertex_count; v++)
			{
				...
				if(is_print_invalid || vertex >= getVertexCount())
				{
					if(DEBUGGING_VALIDATE)
						cout << "Invalid vertex in \"" << m_file_name << "\": Polyline " << l << ", vertex " << v << " has vertex " << (vertex + 1) << " / " << getVertexCount()  << endl;
					m_valid = false;
				...
				{
					if(is_print_invalid || DEBUGGING_VALIDATE)
						cout << "Invalid texture coordinates in \"" << m_file_name << "\": Polyline " << l << ", vertex " << v << " has texture coordinates " << (texture_coordinates + 1) << " / " << getTextureCoordinateCount()  << endl;
					m_valid = false;
		...
		for(unsigned int f = 0; f < (unsigned int)(mv_meshes[m].mv_faces.size()); f++)
		{
			unsigned int face_vertex_count = (unsigned int)(mv_meshes[m].mv_faces[f].mv_vertexes.size());
			if(face_vertex_count < 3)
			{
				if(is_print_invalid || DEBUGGING_VALIDATE)
					cout << "Invalid vertex in \"" << m_file_name << "\": Mesh " << m << ", face " << f << " has only " << face_vertex_count << " vertexes (min 3)" << endl;
				m_valid = false;
			...
			for(unsigned int v = 0; v < face_vertex_count; v++)
			{
				...
				{
					if(is_print_invalid || DEBUGGING_VALIDATE)
						cout << "Invalid vertex in \"" << m_file_name << "\": Mesh " << m << ", face " << f << ", vertex " << v << " has vertex " << (vertex + 1) << " / " << getVertexCount() << endl;
					m_valid = false;
				...
				{
					if(is_print_invalid || DEBUGGING_VALIDATE)
						cout << "Invalid texture coordinates in \"" << m_file_name << "\": Mesh " << m << ", face " << f << ", vertex " << v << " has texture coordinates " << (texture_coordinates + 1) << " / " << getTextureCoordinateCount()  << endl;
					m_valid = false;
				...
				{
					if(is_print_invalid || DEBUGGING_VALIDATE)
						cout << "Invalid normal in \"" << m_file_name << "\": Mesh " << m << ", face " << f << ", vertex " << v << " has normal " << (normal + 1) << " / " << getNormalCount()  << endl;
					m_valid = false;
...
ObjVbo<float> ObjModel :: createPointSetVboVertexOnly (unsigned int mesh) const
{
	...
	for(unsigned int p = 0; p < (unsigned int)(mv_meshes[mesh].mv_point_sets.size()); p++)
	{
		const vector<unsigned int>& v_vertex_ids = mv_meshes[mesh].mv_point_sets[p].mv_vertexes;
		for(unsigned int i = 0; i < (unsigned int)(v_vertex_ids.size()); i++)
		{
			assert(i < (unsigned int)(v_vertex_ids.size()));
			assert(v_vertex_ids[i] < (unsigned int)(mv_vertexes.size()));
                                                       ...
                                                       unsigned int polyline) const
{
	...
	unsigned int                  vertex_count = (unsigned int)(v_vertex_ids.size());
	PositionOnly*                 d_vertexes   = new PositionOnly[vertex_count];

	for(unsigned int i = 0; i < (unsigned int)(v_vertex_ids.size()); i++)
	{
		assert(i < (unsigned int)(v_vertex_ids.size()));
		assert(v_vertex_ids[i].m_vertex < (unsigned int)(mv_vertexes.size()));
                                                                    ...
                                                                    unsigned int polyline) const
{
	...
	for(unsigned int i = 0; i < (unsigned int)(v_vertex_ids.size()); i++)
	{
		...
		if(v_vertex_ids[i].m_texture_coordinate < (unsigned int)(mv_texture_coordinates.size()))
		{
                                                   ...
                                                   const VertexArrangement& vv_arrangement) const
{
	...
	assert(mesh < getMeshCount());
	assert(getFaceCount(mesh) > 0);
	assert((unsigned int)(vv_arrangement.size()) == getVertexCount());
	...
	for(unsigned int i = 0; i < (unsigned int)(vv_arrangement.size()); i++)
		if(!vv_arrangement[i].empty())
		{
			assert(vv_arrangement[i].size() == 1); // cannot be more because all the same

			assert(i < (unsigned int)(mv_vertexes.size()));
                                                                ...
                                                                const VertexArrangement& vv_arrangement) const
{
	...
	unsigned int next_vertex = 0;
	for(unsigned int i = 0; i < (unsigned int)(vv_arrangement.size()); i++)
		for(unsigned int j = 0; j < (unsigned int)(vv_arrangement[i].size()); j++)
		{
			...
			if(element.m_texture_coordinate < (unsigned int)(mv_texture_coordinates.size()))
			{
                                                     ...
                                                     const VertexArrangement& vv_arrangement) const
{
	...
	for(unsigned int i = 0; i < (unsigned int)(vv_arrangement.size()); i++)
		for(unsigned int j = 0; j < (unsigned int)(vv_arrangement[i].size()); j++)
		{
			...
			if(element.m_normal < (unsigned int)(mv_normals.size()))
			{
...
ObjVbo<unsigned int> ObjModel :: createIndexVboSimple (unsigned int count) const
{
	...
	ObjVbo<unsigned int> vbo(GL_ELEMENT_ARRAY_BUFFER, GL_STATIC_DRAW, count, d_indexes);
	delete[] d_indexes;
                                                         ...
                                                         const VertexArrangement& vv_arrangement) const
{
	...
	assert(mesh < (unsigned int)(mv_meshes.size()));
	const vector<Face>& v_faces = mv_meshes[mesh].mv_faces;
	...
	for(unsigned int v = 0; v < (unsigned int)(vv_arrangement.size()); v++)
	{
		assert(v < (unsigned int)(v_start.size()));
		v_start[v] = next_start;
		next_start += (unsigned int)(vv_arrangement[v].size());
	}
	...
	for(unsigned int f = 0; f < (unsigned int)(v_faces.size()); f++)
	{
		unsigned int face_vertex_count = (unsigned int)(v_faces[f].mv_vertexes.size());
	...
	for(unsigned int f = 0; f < (unsigned int)(v_faces.size()); f++)
	{
		...
		for(unsigned int t = 2; t < (unsigned int)(v_vertex_ids.size()); t++)  // per triangle
			for(unsigned int i = 0; i < 3; i++)  // 3 vertexes in each triangle
			{
				...
				for(unsigned int j = 0; j < (unsigned int)(v_vertex_uses.size()); j++)
				{
					...
					assert(face_vertex.m_vertex < (unsigned int)(v_start.size()));
	...
	ObjVbo<unsigned int> vbo(GL_ELEMENT_ARRAY_BUFFER, GL_STATIC_DRAW, vertex_count_total, d_indexes);
	delete[] d_indexes;
                                             ...
                                             ObjModel::VertexArrangement& rvv_arrangement) const
{
	...
	rvv_arrangement.resize((unsigned int)(mv_vertexes.size()));
	...
	for(unsigned int f = 0; f < (unsigned int)(v_faces.size()); f++)
	{
		...
		for(unsigned int i = 0; i < (unsigned int)(v_vertex_ids.size()); i++)
		{
			...
			assert(face_vertex.m_vertex < (unsigned int)(rvv_arrangement.size()));
			...
			if(is_texture_coordinates)
			{
				if(is_normals)
				{
					for(unsigned int j = 0; j < (unsigned int)(rv_vertex_uses.size()); j++)
	...
	if(DEBUGGING_FACE_SHADERS)
	{
		...
		for(unsigned int v = 0; v < (unsigned int)(rvv_arrangement.size()); v++)
		{
			cout << "\t" << v << ":";
			if(!rvv_arrangement[v].empty())
			{
				cout << "\t" << mv_vertexes[v] << endl;
				for(unsigned int i = 0; i < (unsigned int)(rvv_arrangement[v].size()); i++)
				{
...
unsigned int ObjModel :: getVertexArrangementTotalCount (const VertexArrangement& vv_arrangement)
{
	...
	for(unsigned int i = 0; i < (unsigned int)(vv_arrangement.size()); i++)
		count += (unsigned int)(vv_arrangement[i].size());
	return count;
}
...
bool ObjModel :: readPointSet (const string& str, ostream& r_logstream)
{
	...
	{
		assert(mv_meshes.size() >= 1);
		mesh_index = (unsigned int)(mv_meshes.size()) - 1;
	}
...
{
	if(!ObjStringParsing::isValidFilename(m_file_name)) return false;
