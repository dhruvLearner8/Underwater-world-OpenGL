...
//           doubles.
//  Side Effect: N/A
//  Warning: This function assumes that the memebr fields are
...
//
	bool isZero () const
	{
		if(std::fabs(x) > VECTOR3_ZERO_TOLERENCE) return false;
		if(std::fabs(y) > VECTOR3_ZERO_TOLERENCE) return false;
		if(std::fabs(z) > VECTOR3_ZERO_TOLERENCE) return false;
		return true;
	}
...
//
	bool isNormal () const
	{
		...
		return (std::fabs(norm_sqr_minus_1) <
		        VECTOR3_NORM_TOLERANCE_SQUARED);
	}
...
//
	double getChessboardDistance (
	                             const Vector3& other) const
	{
		double dx = std::fabs(x - other.x);
		double dy = std::fabs(y - other.y);
		double dz = std::fabs(z - other.z);
...
//
	bool isParallel (const Vector3& other) const
	{
		...
		if(isZero() || other.isZero())
			return true;
		else if(std::fabs(getNormalized().getCosAngleNormal(
		                               other.getNormalized())) >
		        VECTOR3_ONE_MINUS_NORM_TOLERANCE_SQUARED)
		{
			return true;
...
//
	bool isParallelNormal (const Vector3& other) const
	{
		...
		if(std::fabs(getCosAngleNormal(other)) >
		   VECTOR3_ONE_MINUS_NORM_TOLERANCE_SQUARED)
		{
			return true;
		}
...
//  Side Effect: N/A
//
	Vector3 getRejection (const Vector3& project_onto) const
	{
		assert(isFinite());
		assert(project_onto.isFinite());
		assert(!project_onto.isZero());
...
//           x-axis in radians.  (0, 1, 0) is 0 radians.
//  Side Effect: N/A
//
...
//    <1> isFinite()
//  Returns: The current rotation of this Vector3 around the
//           x-axis in radians.  (0, 1, 0) is 0 radians.  If
//           y == 0.0 && z == 0.0, 0.0 is returned.
//  Side Effect: N/A
//
...
//    <2> z != 0.0 || x != 0.0
//  Returns: The current rotation of this Vector3 around the
//           y-axis in radians.  (0, 0, 1) is 0 radians.
//  Side Effect: N/A
//
...
//           are 0.0, but it is slower than the getRotationY
//           function.
//  Parameter(s): N/A
//  Precondition(s):
//    <1> isFinite()
//  Returns: The current rotation of this Vector3 around the
//           y-axis in radians.  (0, 0, 1) is 0 radians.  If
//           x == 0.0 && z == 0.0, 0.0 is returned.
//  Side Effect: N/A
//
...
//    <2> x != 0.0 || y != 0.0
//  Returns: The current rotation of this Vector3 around the
//           z-axis in radians.  (1, 0, 0) is 0 radians.
//  Side Effect: N/A
//
...
//    <1> isFinite()
//  Returns: The current rotation of this Vector3 around the
//           z-axis in radians.  (1, 0, 0) is 0 radians.  If
//           x == 0.0 && y == 0.0, 0.0 is returned.
//  Side Effect: N/A
//
...
//
	                                  ...
	                                  const Vector3& axis) const
	{
		...
		Vector3 best_possible =
		                       desired.getRejectionNormal(axis);
...
//
//  Purpose: To generate a Vector3 based on the specified seed
...
//           output stream.
//  Parameter(s):
//    <1> r_os: The output stream to print to
//    <2> vector: The Vector3 to print
//  Precondition(s): N/A
//  Returns: A reference to r_os.
//  Side Effect: vector is printed to r_os in tuple notation
//               (e.g. "(1, 2, 3)").
//
...
//
//  Purpose: To read values from the specified input stream into
//           the specified Vector3.  These can be 3 simple values
//           (e.g. "1, 2, 3") or values in tuple notation
//           (e.g. "(1, 2, 3)").
//  Parameter(s):
//    <1> r_is: The input stream
//    <2> r_vector: The Vector3 to read into
//  Precondition(s): N/A
//  Returns: A reference to r_is.
//  Side Effect: r_vector is set to the values in r_is.  The
//               values read are removed from r_is.  If the
//               vector was in tuple notation, the closing
//               parenthesis is also removed.
//
std::istream& operator>> (std::istream& r_is,
                          Vector3& r_vector);

//
//  floor
//
//  Purpose: To calculate the floor of each component in the
//           specified Vector3.
//  Parameter(s):
//    <1> vector: The Vector3
//  Precondition(s): N/A
//  Returns: A Vector3 containing the components
//           (floor(vector.x), floor(vector.y), floor(vector.z)).
//  Side Effect: N/A
//
inline Vector3 floor (const Vector3& vector)
{
	return Vector3(std::floor(vector.x),
	               std::floor(vector.y),
	               std::floor(vector.z));
}
...
//  ceil
//
//  Purpose: To calculate the ceiling of each component in the
//           specified Vector3.
//  Parameter(s):
//    <1> vector: The Vector3
//  Precondition(s): N/A
//  Returns: A Vector3 containing the components
//           (ceil(vector.x), ceil(vector.y), ceil(vector.z)).
//  Side Effect: N/A
//
inline Vector3 ceil (const Vector3& vector)
{
	return Vector3(std::ceil(vector.x),
	               std::ceil(vector.y),
	               std::ceil(vector.z));
}
...
//  round
//
//  Purpose: To calculate the rounded value of each component in
//           the specified Vector3.
//  Parameter(s):
//    <1> vector: The Vector3
//  Precondition(s): N/A
//  Returns: If C++ 11 is available,,this function returns a
//           Vector3 containing the components
//           (round(vector.x), round(vector.y), round(vector.z)).
//           Otherwise it attempts to round each component to
//           the integer value that is nearest to x, with
...
{
#if __cplusplus >= 201103L  // C++11
	return Vector3(std::round(vector.x),
	               std::round(vector.y),
	               std::round(vector.z));
#else
	double rounded_x = (vector.x < 0.0) ?
	                   std::ceil (vector.x - 0.5) :
	                   std::floor(vector.x + 0.5);
	double rounded_y = (vector.y < 0.0) ?
	                   std::ceil (vector.y - 0.5) :
	                   std::floor(vector.y + 0.5);
	double rounded_z = (vector.z < 0.0) ?
	                   std::ceil (vector.z - 0.5) :
	                   std::floor(vector.z + 0.5);
...
//  abs
//
//  Purpose: To calculate the absolute value of each component
//           in the specified Vector3.
//  Parameter(s):
//    <1> vector: The Vector3
//  Precondition(s): N/A
//  Returns: A Vector3 containing the components
//           (fabs(vector.x), fabs(vector.y), fabs(vector.z)).
//  Side Effect: N/A
//
inline Vector3 abs (const Vector3& vector)
{
	return Vector3(std::fabs(vector.x),
	               std::fabs(vector.y),
	               std::fabs(vector.z));
}
inline Vector3 fabs (const Vector3& vector)
{
	return abs(vector);
}
...
//  fmod
//
//  Purpose: To calculate the modulus of each component in the
//           specified Vector3 with the specified scalar value.
//  Parameter(s):
//    <1> dividend: The dividend
//    <2> divisor: The divisor
//  Precondition(s):
//    <1> divisor != 0.0
//  Returns: A Vector3 containing the components
//           (fmod(dividend.x, divisor),
//            fmod(dividend.y, divisor),
//            fmod(dividend.z, divisor)).
//  Side Effect: N/A
//
inline Vector3 fmod (const Vector3& dividend,
                     double divisor)
{
	return Vector3(std::fmod(dividend.x, divisor),
	               std::fmod(dividend.y, divisor),
	               std::fmod(dividend.z, divisor));
}

//
//  fmod
//
//  Purpose: To calculate the modulus of each corresponding pair
//           of components in the specified Vector3s.
//  Parameter(s):
//    <1> dividend: The dividend
//    <2> divisor: The divisor
//  Precondition(s):
//    <1> divisor != 0.0
//  Returns: A Vector3 containing the components
//           (fmod(dividend.x, divisor.x),
//            fmod(dividend.y, divisor.y),
//            fmod(dividend.z, divisor.z)).
//  Side Effect: N/A
//
inline Vector3 fmod (const Vector3& dividend,
                     const Vector3& divisor)
{
	return Vector3(std::fmod(dividend.x, divisor.x),
	               std::fmod(dividend.y, divisor.y),
	               std::fmod(dividend.z, divisor.z));
}

//
//  fmod
//
//  Purpose: To calculate the modulus of the specified scalar
//           value with each component in the specified Vector3.
//  Parameter(s):
//    <1> dividend: The dividend
//    <2> divisor: The divisor
//  Precondition(s):
//    <1> divisor != 0.0
//  Returns: A Vector3 containing the components
//           (fmod(dividend, divisor.x),
//            fmod(dividend, divisor.y),
//            fmod(dividend, divisor.z)).
//  Side Effect: N/A
//
inline Vector3 fmod (double dividend,
                     const Vector3& divisor)
{
	return Vector3(std::fmod(dividend, divisor.x),
	               std::fmod(dividend, divisor.y),
	               std::fmod(dividend, divisor.z));
}
